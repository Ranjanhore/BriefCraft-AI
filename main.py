...
def update_render_job(cur, job_id: str, status: str, output_json: Any = None, error_text: Optional[str] = None) -> None:
    ...
    cur.execute("""
        ...
        where id = %s
    """, (status, json.dumps(output_json) if output_json is not None else None, error_text, started_at, finished_at, job_id))
...
@with_db
def create_generation(cur, project_id: str, user_id: str, prompt: str, expanded_prompt: str, model: str, size: str,
                      action_type: str, parent_generation_id: Optional[str] = None,
                      source_image_url: Optional[str] = None, settings: Optional[Dict[str, Any]] = None) -> str:
...
def add_generation_images(cur, generation_id: str, image_urls: List[str]) -> None:
    for idx, url in enumerate(image_urls):
        cur.execute("insert into public.generation_images (id, generation_id, image_index, image_url) values (%s, %s, %s, %s)",
                    (str(uuid.uuid4()), generation_id, idx, url))


@with_db
def complete_generation(cur, generation_id: str, image_urls: List[str]) -> None:
    cur.execute("update public.generations set status = %s, output_images = %s, completed_at = now() where id = %s",
                ("done", json.dumps(image_urls), generation_id))


@with_db
def fail_generation(cur, generation_id: str, error_text: str) -> None:
    cur.execute("update public.generations set status = %s, error_text = %s, completed_at = now() where id = %s",
...
def update_session_slide(cur, session_id: str, slide_no: int) -> None:
    cur.execute("update public.presentation_sessions set current_slide_no = %s, updated_at = now() where id = %s",
                (slide_no, session_id))
...
def get_recent_voice_logs(cur, session_id: str, limit: int = 8) -> List[Dict[str, Any]]:
    cur.execute("select role, message_text from public.voice_interaction_logs where session_id = %s order by created_at desc limit %s",
                (session_id, limit))
...
def run_pipeline(payload: RunInput, user_id: str = Depends(get_current_user_id)):
    project_id = payload.project_id or create_project(user_id=user_id, name=payload.name or best_project_name_from_prompt(payload.text), event_type=payload.event_type)
    project = get_project(project_id)
    if not project:
        raise HTTPException(status_code=404, detail="Project not found")
...
def generate_multi_angle(payload: Generate3DInput, background_tasks: BackgroundTasks, user_id: str = Depends(get_current_user_id)):
    ...
    job_id = create_render_job(project_id=payload.project_id, user_id=user_id, job_type="multi_angle_render", input_json={"scene_json": scene_json, "width": payload.width, "height": payload.height})
    background_tasks.add_task(run_blender_multi_angle, scene_json, job_id, payload.width, payload.height, payload.project_id, user_id)
...
def build_departments(project_id: str, user_id: str = Depends(get_current_user_id)):
    ...
    return {"message": "Departments generated", "project_id": project_id, "sound_data": sound, "lighting_data": lighting, "showrunner_data": showrunner}
...
def presentation_create(payload: PresentationCreateInput, user_id: str = Depends(get_current_user_id)):
    ...
    if not project:
        raise HTTPException(status_code=404, detail="Project not found")
    presentation_id = create_presentation(payload.project_id, user_id, payload.title or f"{project.get('name', 'Project')} Presentation", payload.presenter_name, payload.presenter_style, payload.voice)
...
def presentation_slide_upsert(payload: PresentationSlideInput, user_id: str = Depends(get_current_user_id)):
    _ = user_id
    if not get_presentation(payload.presentation_id):
...
def presentation_start_live(presentation_id: str, user_id: str = Depends(get_current_user_id)):
    if not get_presentation(presentation_id):
        raise HTTPException(status_code=404, detail="Presentation not found")
    session_id = create_presentation_session(presentation_id, user_id, 1)
...
def presentation_ask_audio(session_id: str, file: UploadFile = File(...), language: str = "en", user_id: str = Depends(get_current_user_id)):
    session = get_presentation_session(session_id)
    if not session:
        raise HTTPException(status_code=404, detail="Session not found")
    presentation = get_presentation(session["presentation_id"])
...
async def presentation_realtime_session(request: FastAPIRequest, presentation_id: str, slide_no: int = 1, voice: str = "marin", audience_mode: str = "client", language: str = "en", user_id: str = Depends(get_current_user_id)):
    presentation = get_presentation(presentation_id)
    if not presentation:
        raise HTTPException(status_code=404, detail="Presentation not found")
    ...
    session_id = create_or_get_live_presentation_session(presentation_id, user_id, slide_no)
    ...
    r = requests.post("https://api.openai.com/v1/realtime/calls", headers={"Authorization": f"Bearer {OPENAI_API_KEY}"}, files=files, timeout=60)
